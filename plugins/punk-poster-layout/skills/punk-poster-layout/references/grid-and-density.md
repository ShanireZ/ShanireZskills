...
```text
使用 3 列 × 4 行网格；通过 :nth-child(even) 反转前景/背景；主标题块 grid-column:span 2；为小屏保留 3 列，缩减内部文字而非改成单列。
```
...
```text
建立背景文字层、图像层、前景文字层三个 z-index；同一标题拆成两个可访问副本时给装饰副本 aria-hidden="true"；主体可用透明 PNG 或 clip-path，避免仅靠 opacity。
```
...
先建立可感知的网格，再让少数关键元素跨线、错位或越界，以破坏制造强调。
...
```text
竖版实验海报，先建立清晰的 5 栏网格与重复对齐线，再让标题“[主标题]”跨出两条栏线、主视觉[对象]越过一个模块边界；其余信息严格归位，用少量控制性错位制造焦点。避免所有元素随机漂浮。
...
```text
底层使用 5 栏 Grid；80% 元素遵守 grid lines，焦点元素通过 grid-column 跨栏并 translateX(8%–15%)；避免全部使用 absolute；调试时显示网格覆盖层。
```
